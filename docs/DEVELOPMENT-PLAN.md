...
| **P5** ✅ | 事件流（可验证的沉默） | 哈希链 + 跨交付比对 | 见 §10（删行/改行/砍尾巴都能被指出；跨次改写记为一个事件） |
...
| 委派边界与遏制（P4） | ✅ 见 §9 |
| 事件流 / 可验证的沉默（P5） | ✅ 见 §10 |
| **待定**：这件事还差一步才算"能卖" | `spec/` 的四条新规则需要外部评审；评审稿见 [ASAS-SPEC-for-review.md](ASAS-SPEC-for-review.md) |
...
| # | 任务 | 状态 | 判据 / 证据 |
|---|---|---|---|
| T20 | 事件流规范化哈希写进规范（互操作的前提） | ✅ | spec §6.2：seq 从 0、首条 `prevHash` 为空串、`sha256(canonical_json(event \ {hash}))` |
| T21 | 造链 + 验链各一份实现 | ✅ | Go `internal/chain` 造链；Python `asas.rule_silence_auditable` 验链；跨语言向量 `spec/testvectors/silence/events.jsonl` 两边各有一个"必须与向量逐字相同"的用例 |
| T22 | `deliver` 自动出事件流并纳入证据 | ✅ | 第 ②b 步写 `delivery/events.jsonl`；哈希进 manifest；本地自查 `silenceIsAuditable` 由"未评估"变 ✅ |
| T23 | 控制面：事件上传 + 跨交付比对 | ✅ | `POST /events`、`GET /silence`；比的是**重算值**与上次的逐条哈希，不是事件自称的 `hash` |
| T24 | 传感器带上事件流 | ✅ | 从 `<SCAN_HOME>/.ratchet/calls.jsonl` 取；检出断流时**非 0 退出** |
| T25 | 落档 | ✅ | [DELIVERY-0.18.0.md](DELIVERY-0.18.0.md) |

**这一轮最重要的一句结论**：

> **V6 只能证明"这条链内部没被动过"，不能证明"它就是当时那条链"。**

砍尾巴、或改完重新算链，得到的都是自洽的链，规则会判通过。能补上这一半的只有一样东西——
**此前被引用过的摘要**。所以跨次比对不是锦上添花：它是"可验证的沉默"的另一半，
而且天然属于持有台账的那一方。
