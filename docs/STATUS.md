...
* 可访问性实测（2026-09-26，**量的是静态产物**；命令是 `cd web && node scripts/check-pages.mjs <baseUrl>`，
  路由从产物目录扫、不手写）：18 条路由 × 390px 与 1440px，
  横向溢出 0、越界元素 0、小于 24px 的点击目标 0；全站文字对比度 0 处低于 WCAG AA；
  18 条路由**全部水合成功**（见下面那条，水合本身就是这一轮查出来的故障）。
  早前那轮（2026-09-25）修掉了两处文档与实现的漂移：正文/元信息色低于 AA、主按钮白字只有 4.33:1。
* **静态产物的两个坑（2026-09-26 实测踩到并修掉）**：
  1. 路由必须**显式登记**预渲染（`web/routes.ts`）。`nuxt generate` 默认靠爬页面链接发现路由，
     而 `/thanks` 是支付成功后的跳转目标、站内没有入口指向它，于是从没被生成——
     客户点完付款落在 404 上。现在清单与 `app/pages/` 双向对照，由测试兜住。
  2. CSP 的 `script-src 'self'` 把 Nuxt 写的**内联**脚本（运行时配置 + payload + importmap）
     全拦了。症状不是白屏，而是**整站不水合**：HTML 一切正常、文字都在，
     但收银台按钮点了没反应、`/verify` 的哈希算不出来。现在按构建产物算 sha256 逐个放行
     （`deploy/docker/render-security-headers.mjs`），而不是放开 `'unsafe-inline'`。
...
| A5 | `mcpb` 资产缺失 | ~~网站指向 404~~ **已收口**（2026-09-26）：`dist/ratchet-0.13.0.mcpb`（12,210,966 字节，含四平台二进制）已传到 `v0.13.0` release，`desktopBundleUrl` 填回；下载复验 sha256 一致 | 已满足：`releases/latest/download/ratchet-0.13.0.mcpb` 返回 200 且哈希对得上 |
